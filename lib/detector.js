...
function createDetector(onSignal, options = {}) {
  ...
  function checkForChanges() {
    ...
    try {
      // 检查文件是否存在
      if (!fs.existsSync(SIGNAL_FILE)) {
        if (verbose && checkCount % 10 === 0) {
          console.log('[检测] 信号文件不存在，尝试创建...');
        }
        ensureSignalFile();
        return;
      }
      ...
      if (verbose && checkCount % 10 === 0) {
        console.log(`[检测] 文件行数: ${lines.length}, 上次: ${lastLine ? '有' : '无'}`);
      }
      ...
      if (signal && signal.event === 'api_error') {
        console.log('[解析成功]', JSON.stringify(signal));
        onSignal(signal);
      } else {
        console.log('[解析结果]', signal);
      }
    } catch (e) {
      if (verbose) {
