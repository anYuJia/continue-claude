...
function sendWindows(text) {
  return new Promise((resolve, reject) => {
    ...
    exec(`type "${tempFile}" | clip`, { shell: true }, (err) => {
      ...
      // Step 2: Send Ctrl+V and Enter using PowerShell script file
      // Avoid cmd escaping issues by using a script file
      const psFile = path.join(os.tmpdir(), 'send-keys.ps1');
      const psContent = `
...
`.trim();
      ...
      exec(`powershell -NoProfile -ExecutionPolicy Bypass -File "${psFile}"`, { shell: true }, (err2) => {
        try { fs.unlinkSync(tempFile); } catch (e) {}
