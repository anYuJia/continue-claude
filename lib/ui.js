...
function render(state) {
  ...
  if (state.signalFile) {
    lines.push(`  │ 信号文件: ${colors.dim}${state.signalFile}${colors.reset}`);
  }
  lines.push(`  ${'─'.repeat(Math.min(width - 4, 60))}`);
  ...
  if (processes.length === 0) {
    ...
    for (const p of processes.slice(0, 8)) { // 最多显示8个
  ...
  // 底部提示
  lines.push('');
  lines.push(`  ${colors.dim}按 Ctrl+C 退出 | --test 测试信号${colors.reset}`);
